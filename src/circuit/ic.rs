...
impl Component for Ic
{
	...
	fn input_type(&self, input: usize) -> Option<InputType> {
		...
		self.inputs.get(input).map(|_| InputType { bits: core::num::NonZeroU8::new(1).unwrap() })
	}
	...
	fn output_type(&self, output: usize) -> Option<OutputType> {
		// TODO don't hardcode bits.
		self.outputs.get(output).map(|_| OutputType { bits: core::num::NonZeroU8::new(1).unwrap() })
	}

	fn generate_ir(&self, inputs: &[usize], outputs: &[usize], out: &mut dyn FnMut(ir::IrOp), memory_size: usize) -> usize {
		out(ir::IrOp::RunIc { ic: self.ir.clone().into(), offset: memory_size, inputs: inputs.into(), outputs: outputs.into() });
		self.memory_size
	}
}

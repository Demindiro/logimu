...
pub enum IrOp {
	...
	RunIc { ic: Rc<[Self]>, offset: usize, inputs: Box<[usize]>, outputs: Box<[usize]> },
}

impl fmt::Debug for IrOp {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let fmt1 = |f: &mut fmt::Formatter, op, a, b| {
			write!(f, "{:>3} = ({:<5} {:>3})", b, op, a)
		};
		...
		match self {
			...
			IrOp::RunIc { ic, offset, inputs, outputs } => write!(f, "{:>3?} (runic {:>3} {:>3p} {:>3?})", &inputs, offset, Rc::as_ptr(ic), &outputs),
		}
	}
}

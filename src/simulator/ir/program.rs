...
fn run(ops: &[IrOp], rd: &[usize], wr: &mut [usize], dirty: &mut IntegerSet) {
	...
	for op in ops {
		match op {
			...
			&IrOp::Xor { a } => acc ^= rd[a],
			&IrOp::Not => acc = !acc,
			&IrOp::NotAnd { a } => acc &= !rd[a],
			&IrOp::NotOr { a } => acc |= !rd[a],
			&IrOp::NotXor { a } => acc ^= !rd[a],
			&IrOp::Andi { i } => acc &= i,
			&IrOp::Xori { i } => acc ^= i,
			&IrOp::Slli { i } => acc <<= i,
			&IrOp::Srli { i } => acc >>= i,
			&IrOp::Copy { a } => acc = rd[a],
			&IrOp::CopyNot { a } => acc = !rd[a],
...
impl fmt::Debug for IrOp {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		...
		match self {
			...
			IrOp::Not => fmt0(f, "not"),
			IrOp::NotAnd { a } => fmt1(f, "not-and", a),
			IrOp::NotOr { a } => fmt1(f, "not-or", a),
			IrOp::NotXor { a } => fmt1(f, "not-xor", a),
			...
			IrOp::CopyNot { a } => fmt1(f, "copy-not", a),
